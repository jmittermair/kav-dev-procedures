...
#Can we run these both in a single command whilst supporting MFA? Maybe partner center module is the answer.
...
$skuids = @{
    ...
    'CCIBOTS_PRIVPREV_VIRAL'='Dynamic Bots Trial';
    ...
    'MCOPSTNC_MCOPSTNC'='Communications Credits';
    ...
    'STANDARDWOFFPACK_FACULTY'='Office 365 Education E2 for Faculty';
    ...
    'STANDARDWOFFPACK_STUDENT'='Office 365 Education E2 for Students';
...
Get-MsolAccountSku | Foreach-Object { 
  $ObjectArray += [PSCustomObject]@{
    LicenseName = $Skuids["$($_.AccountSkuId.Split(':')[1])"]
    ActiveUnits = $_.ActiveUnits
    ConsumedUnits = $_.ConsumedUnits
  }
}

#ToDo: Export object to CSV and ask user running script where to save it to

$NumSharedMailboxes = (Get-Mailbox | ? {$_.RecipientTypeDetails -eq "SharedMailbox"}).Length
Write-Host -ForegroundColor "Blue" "There are $($numSharedMailboxes) currently active"
